...
def print_phb h_sum_phb, h_code
	...
	h_sum_phb.each_key { |key|
		puts "#{i}\t#{key}\t#{h_sum_phb[key][:name]}\t#{h_sum_phb[key][:nhsy].round(2)}\t#{h_sum_phb[key][:fenhong].round(2)}\t#{h_sum_phb[key][:songgu]}\t#{h_code[key][:pe]}\t#{h_code[key][:hangye]}\t#{h_code[key][:ltgb].round(2)}\t#{h_sum_phb[key][:date].uniq.sort}"
		i = i + 1
