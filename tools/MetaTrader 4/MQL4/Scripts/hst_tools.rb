...
class TDXRecordlc5 < BinData::Record
  uint16le :date
  uint16le :time
  float_le :open
  float_le :high
...
# dayFile = "D:\\dev\\ruby\\mt4\\ds\\lday\\33#399001.day"
dayFile = "D:\\dev\\ruby\\mt4\\ds\\sz\\sz002508.lc5"
io = File.open(dayFile)

10.times {
	rd = TDXRecordlc5.read(io)
	puts "TDXRecordlc5: date is #{(rd.date/2048).floor+2004}-#{((rd.date%2048)/100).floor}-#{(rd.date%2048)%100}, time is #{rd.time/60}:#{rd.time%60}, open is #{rd.open.to_f}, high is #{rd.high.to_f}, low is #{rd.low.to_f}, close is #{rd.close.to_f}, amount is #{rd.amount}, vol is #{rd.vol}, reservation is #{rd.reservation}"
}
